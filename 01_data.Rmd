...
# Look at TAU data
```{r tau numbers}
# The breakdown in the dates data for TAU
table(dates[dates$arm == 2, ]$tele) 
# 47 in-person, 50 telehealth, 9 mixed

# Filter the dataset to TAU 
tau_data_filtered <- plf_data %>%
  filter(
    arm_label == "TAU",
    month != -9
  )

# Add in tele, dtcon, and dtm00 from "dates"
tau_data_filtered <- tau_data_filtered %>%
  left_join(
    dates %>% select(id, tele, dtcon, dtm00),  
    by = "id"                                    
  )
```
...
```{r save tau data, eval=FALSE}
write.csv(
  tau_data_filtered,
  paste0("01_Output/tau_data_sg_", Sys.Date(), ".csv")
)
# 2026-01-05: First filtered data
```
