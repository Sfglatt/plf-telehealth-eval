...
# Chi-square between n_attended (0 vs >0) and modality (1 vs 2; 3 should be 1)

```{r chi sq attend mode}
# Chi-square between n_attended (0 vs >0) and modality (1 vs 2; 3 should be 1)
dates$initiated <- ifelse(dates$id %in% sessions$id[sessions$n_attended > 0], 1, 0)
dates$tele_binary <- ifelse(dates$tele == "1", 1,
                            ifelse(dates$tele == "2", 2,
                                   ifelse(dates$tele == "3", 1, NA))) # "mixed" were assigned in-person

chi_square_result <- chisq.test(dates[dates$arm == 1,]$initiated, dates[dates$arm == 1,]$tele_binary, correct = FALSE)
chi_square_result
report::report(chi_square_result)

# Table
sjPlot::tab_xtab(
  var.row = dates[dates$arm == 1,]$initiated,
  var.col = dates[dates$arm == 1,]$tele_binary,
  title = "Contingency table of Treatment Initiation by Modality Type",
  show.row.prc = TRUE,
  show.col.prc = TRUE,
  show.summary = TRUE,
  statistics = "pearson",
  show.exp = TRUE,
  show.legend = TRUE,
  file = paste0("01_Output/Contingency_initiation_modality_", Sys.Date(), ".doc")
)
```
