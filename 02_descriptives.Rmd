...
# 22 in person, 44 telehealth, 4 mixed-modal
# 26 assigned in person, 44 assigned in telehealth

# Remove mixed-modal participants from baseline dataset
plf_data_baseline_sens <- plf_data_baseline %>%
  filter(modality_type != 3)

# Check how many are missing baseline suicidal ideation (covariate)
...
# 46 assigned in-person, 55 assigned in telehealth
```
...
table(plf_data_baseline_sens$Attendance_clean)
...
describe_tidy <- function(data, modality_label) {
  ...
  bind_rows(lapply(names(desc_list), function(m) {
    ...
    out <- data.frame(
      ...
      n = tbl$n,
      `Mean (SD)` = sprintf("%.2f (%.2f)", tbl$mean, tbl$sd),
      `Median [Range]` = sprintf(
        "%.2f [%.2f - %.2f]",
        tbl$median,
        tbl$min,
        tbl$max
      ),
      skew = round(tbl$skew, 2),
      kurtosis = round(tbl$kurtosis, 2),
...
# Make mixed-modal into in-person because this is baseline descriptives/cohort characterization and they were assigned in-person. We will keep them in the ITT sample as assigned (in-person) but remove them from the MLM sample since they are a mix of both modalities and we want to characterize the two groups as cleanly as possible for the MLM sample.
plf_data_baseline <- plf_data_baseline %>%
  mutate(modality_type_assign = if_else(modality_type == 3, 1, modality_type))

table(plf_data_baseline$modality_type_assign)

# Isolate data
covariates_mlm <- plf_data_baseline %>%
  dplyr::select(
    modality_type_assign,
    bhs, atspph, bdi, ssi, inq_pb, inq_tb, srcs17, srcse, srcsi,
...
covariates_mlm <- psych::describeBy(covariates_mlm[, -1], covariates_mlm$modality_type_assign)
in_person_table_mlm <- covariates_mlm$"1"
telehealth_table_mlm <- covariates_mlm$"2"
...
t_test_covariates_mlm <- jmv::ttestIS(
  formula = srcs17 + srcse + srcsi + ssi + bhs + atspph + bdi +
    inq_tb + inq_pb + bpaqpa + bpaqva +
    Number_MH_encounters + age_demo ~ modality_type_assign,
  data = plf_data_baseline
  #  %>% dplyr::filter(!is.na(ssi))
  ,
  vars = vars(
    srcs17, srcse, srcsi, ssi, bhs, atspph, bdi,
    inq_tb, inq_pb, bpaqpa, bpaqva,
...
write.csv(
  in_person_table_mlm,
  paste0("02_Output/baseline_table_mlm_inperson_sg_", Sys.Date(), ".csv")
)

write.csv(
  telehealth_table_mlm,
  paste0("02_Output/baseline_table_mlm_telehealth_sg_", Sys.Date(), ".csv")
)

write.csv(
  in_person_table_itt,
  paste0("02_Output/baseline_table_itt_inperson_sg_", Sys.Date(), ".csv")
)

write.csv(
  telehealth_table_itt,
  paste0("02_Output/baseline_table_itt_telehealth_sg_", Sys.Date(), ".csv")
)
```
...
# Demographics
- Full sample assigned to treatment (101)
  Telehealth (55)
  In-person (46)
- Full sample that initiated treatment (70)
  Telehealth (44)
  In-person (26)
```{r demo function}
demo_vars <- list(
  ...
  Attendance_clean = list(
    var = "Attendance_clean",
...
These are demographic tables corresponding to the initial sample, which includes mixed-modal participnts (n=4) in the in-person sample since they were *assigned* in-person. 
```{r demo table paper}
# Demographic table for N = 70
demographic_table_70 <- make_demographic_table(
  data = plf_data_baseline,
  N = 70,
  demo_vars = demo_vars
)

# Demographic table for those who were assigned in-person and initiated
plf_data_baseline_ip <- plf_data_baseline %>%
  dplyr::filter(modality_type_assign == 1)
...
# Demographic table for those who were assiged in telehealth and initiated
plf_data_baseline_th <- plf_data_baseline %>%
  dplyr::filter(modality_type_assign == 2)
...
# Demographic table for full PLF arm
demographic_table_101 <- make_demographic_table(
  data = plf_data_itt,
  N = 101,
  demo_vars = demo_vars
)
...
plf_data_itt_baseline_ip <- plf_data_itt %>%
  dplyr::filter(modality_assign == 0)

demographic_table_46 <- make_demographic_table(
...
plf_data_itt_baseline_th <- plf_data_itt %>%
  dplyr::filter(modality_assign == 1)

demographic_table_55 <- make_demographic_table(
...
# Merge tables
demo_70 <- demographic_table_70 %>%
  rename(`Total received (N = 70)` = `n (%)`)

demo_th <- demographic_table_th %>%
  rename(`Received telehealth (N = 44)` = `n (%)`)

demo_ip <- demographic_table_ip %>%
  rename(`Received in-person (N = 26)` = `n (%)`)

demo_101 <- demographic_table_101 %>%
  rename(`Total assigned (N = 101)` = `n (%)`)

demo_46 <- demographic_table_46 %>%
  rename(`Assigned in-person (N = 46)` = `n (%)`)

demo_55 <- demographic_table_55 %>%
  rename(`Assigned telehealth (N = 55)` = `n (%)`)

demographic_table <- demo_70 %>%
  full_join(demo_th,
...
# Age
# Total initiated treatment (N = 70)
nrow(plf_data_baseline)
summary(plf_data_baseline$age_demo)
sd(plf_data_baseline$age_demo, na.rm = TRUE)

# Telehealth (n = 44)
nrow(plf_data_baseline_th)
summary(plf_data_baseline_th$age_demo)
sd(plf_data_baseline_th$age_demo, na.rm = TRUE)

# In-person (N = 26)
nrow(plf_data_baseline_ip)
summary(plf_data_baseline_ip$age_demo)
sd(plf_data_baseline_ip$age_demo, na.rm = TRUE)

# Total assigned to treatment (N = 101)
nrow(plf_data_itt)
summary(plf_data_itt$age_demo)
sd(plf_data_itt$age_demo, na.rm = TRUE)

# Telehealth assigned (N = 55)
nrow(plf_data_itt_baseline_th)
summary(plf_data_itt_baseline_th$age_demo)
sd(plf_data_itt_baseline_th$age_demo, na.rm = TRUE)

# In-person assigned (N = 46)
nrow(plf_data_itt_baseline_ip)
summary(plf_data_itt_baseline_ip$age_demo)
sd(plf_data_itt_baseline_ip$age_demo, na.rm = TRUE)
```

These are demographic tables corresponding to the multilevel model sample, which excludes mixed-modal participnts (n=4) and those missing baseline suicidal ideation scores (n = 2)
```{r demo table mlm}
...
# Demographic table for those assigned to telehealth
